    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                {
                    Events = await RaceEventSettings.LoadCurrentEventSettings(dbFactory, DateTime.UtcNow, stoppingToken)
                };
            ...
            await Task.Delay(TimeSpan.FromSeconds(double.Parse(Config["CONFIGURATIONCHECKRATESECS"] ?? throw new InvalidOperationException("CONFIGURATIONCHECKRATESECS is required"))), stoppingToken);
        }
    }
}
