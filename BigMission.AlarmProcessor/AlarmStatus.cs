...
class AlarmStatus
{
    ...
    private readonly List<ConditionStatus> conditionStatus = [];
    ...
    public AlarmStatus(CarAlarm alarm, IDbContextFactory<RedMist> dbFactory, ILogger logger, IConnectionMultiplexer cacheMuxer, Func<int, Task<int>> getDeviceId)
    {
        ...
        InitializeConditions([.. alarm.AlarmConditions]);
    }
    ...
    public async Task<bool> CheckConditions(ChannelStatusDto[] channelStatus)
    {
        ...
        {
            alarmOn = results.All(r => r ?? false);
        }
        else if (Alarm.ConditionOption == ANY)
        {
            alarmOn = results.Any(r => r ?? false);
        }
