...
public class ServiceTracking : BackgroundService
{
    ...
    {
        ServiceId = (config["SERVICEID"] ?? throw new InvalidOperationException("REDIS_SVC is required"));
        if (config["ASPNETCORE_ENVIRONMENT"] != "Development")
        {
            ServiceId = (config["HOSTNAME"] ?? throw new InvalidOperationException("REDIS_SVC is required"));
        }
    ...
    private async Task ScanForUpdates()
    {
        ...
        {
            if (!ss.Value.HasValue)
            {
                Logger.LogWarning($"Service {ss.Name} has no status");
                continue;
            }
            var st = JsonConvert.DeserializeObject<ServiceStatus>(ss.Value!);
            if (st == null)
            {
                Logger.LogWarning($"Service {ss.Name} has invalid status");
                continue;
            }
