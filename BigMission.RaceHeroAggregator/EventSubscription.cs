...
class EventSubscription
{
    ...
    private readonly List<CarSubscription> subscriberCars = [];
    private readonly Dictionary<int, RaceEventSetting> settings = [];
        ...
        IDateTimeHelper dateTime, ISimulateSettingsService simulateSettingsService, IDbContextFactory<RedMist> dbFactory)
    {
        ...
        waitForStartInterval = TimeSpan.FromMilliseconds(int.Parse(Config["WAITFORSTARTTIMER"] ?? throw new InvalidOperationException("WAITFORSTARTTIMER is required")));
        eventPollInterval = TimeSpan.FromMilliseconds(int.Parse(Config["EVENTPOLLTIMER"] ?? throw new InvalidOperationException("WAITFORSTARTTIMER is required")));
        logRHToFile = bool.Parse(Config["LogRHToFile"] ?? throw new InvalidOperationException("WAITFORSTARTTIMER is required"));
        readTestFiles = bool.Parse(Config["ReadTestFiles"] ?? throw new InvalidOperationException("WAITFORSTARTTIMER is required"));
    }
    ...
    private async Task ProcessLeaderboardAsync(Leaderboard leaderboard)
    {
        ...
        {
            var sw = Stopwatch.StartNew();
            if (EventId == null || flagStatus == null)
            {
                Logger.LogWarning("EventId or flagStatus is null");
                return;
            }
