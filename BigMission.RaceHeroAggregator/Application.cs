...
class Application : BackgroundService
{
        ...
        ISimulateSettingsService simulateSettingsService, StartupHealthCheck startup, IDbContextFactory<RedMist> dbFactory)
    {
        ...
        var readTestFiles = bool.Parse(Config["ReadTestFiles"] ?? throw new InvalidOperationException("ReadTestFiles is required"));
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var eventSubInterval = TimeSpan.FromMilliseconds(int.Parse(Config["EVENTSUBSCRIPTIONCHECKMS"] ?? throw new InvalidOperationException("ReadTestFiles is required")));
        var waitForStartInterval = TimeSpan.FromMilliseconds(int.Parse(Config["WAITFORSTARTTIMER"] ?? throw new InvalidOperationException("ReadTestFiles is required")));
        var eventPollInterval = TimeSpan.FromMilliseconds(int.Parse(Config["EVENTPOLLTIMER"] ?? throw new InvalidOperationException("ReadTestFiles is required")));
    ...
    private async Task RunSubscriptionCheck(CancellationToken stoppingToken)
    {
        try
        {
            ...
            foreach (var settingGrg in settingEventGrps)
            {
                ...
                await subscription.UpdateSettingAsync([.. settingGrg]);
            }
