...
class DataContext : IDataContext
{
    ...
    public async Task<List<ChannelMapping>> GetConsumptionChannels(int carId)
    {
        var db = await GetDb();
        var virtualChannels = await (from d in db.DeviceAppConfigs
                                     join m in db.ChannelMappings on d.Id equals m.DeviceAppId
                                     where d.CarId == carId && !d.IsDeleted && CarConsumption.ConsumptionChannelNames.Contains(m.ReservedName)
    ...
    public async Task PublishChannelStatus(ChannelDataSetDto channelDataSetDto)
    {
